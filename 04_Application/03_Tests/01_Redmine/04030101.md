...
Menu : Administration → Configuration → onglet **Général**

| Champ | Valeur |
|---|---|
| Titre de l'application | `myRedmine - Dupont` (exemple) |
| Langue par défaut | Français |
| Format de date | %d/%m/%Y |
| Enregistrement des utilisateurs | Désactivé |

Onglet **Projets** : cocher "Suivi du temps" dans les modules activés par défaut.

Cliquer **Sauvegarder** — recharger la page pour vérifier le titre et la langue.
::::
...
**Réponse attendue à l'étape 4 (modification statut)** : code HTTP `200 OK` — corps vide. Vérifier dans l'interface que le statut est bien "En cours" et que la note "Prise en charge via API" apparaît dans l'historique.
