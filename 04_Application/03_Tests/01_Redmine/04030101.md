...
:::{note}
L'option "activation manuelle du compte" empêche tout auto-enregistrement — seul l'administrateur peut créer des comptes. C'est la configuration recommandée en contexte TMA.
:::
...
:::{note}
Décocher "Définir les nouveaux projets comme publics" est essentiel en contexte TMA — les projets doivent être privés par défaut.
:::
...
:::{note}
Cocher "Demande" comme champ obligatoire garantit que chaque saisie de temps est rattachée à un ticket — indispensable pour le reporting TMA.
:::
...
```json
{
  "issue": {
    "id": 42,
    "project": {"id": 1, "name": "TMA Capgemini Engineering"},
