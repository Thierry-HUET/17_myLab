---
title: M2 — Gestion de projet Redmine
short_title: Gestion de projet
authors: 
    - name: Thierry HUET
      affiliations: 
        - APERTO-NOTA
      email: thierry.huet@aperto-nota.fr
subject: Exercices
date: 2026-06-13
---

## Module M2 — Gestion de projet (Jour 2 matin — 3h30)

**Prérequis** : instance configurée par M1 — rôles, trackers, workflow et activités en place  
**Objectif** : créer et piloter un projet Redmine, gérer ses membres et produire des rapports de temps

---

::::{exercise} EX-08 — Création et structuration du projet
:label: redmine_ex08
:nonumber:

**Durée** : 15 min  
**Menu** : Projets → Nouveau projet

| Champ | Valeur |
|---|---|
| Identifiant | `tma-capgemini` |
| Nom | `TMA Capgemini Engineering` |
| Description | Contexte fictif de maintien en condition opérationnelle |
| Modules à activer | Suivi des demandes, Wiki, Documents, Suivi du temps, Fichiers |

Créer ensuite un sous-projet :
- Identifiant : `tma-capgemini-batch`
- Projet parent : `tma-capgemini`

**Critère de réussite** : projet et sous-projet visibles dans l'arborescence
::::

::::{solution} redmine_ex08
:label: sol_redmine_ex08
:class: dropdown

Menu : Projets → **Nouveau projet**

1. Renseigner les champs Nom et Identifiant — l'identifiant est généré automatiquement à partir du nom, vérifier qu'il correspond à `tma-capgemini`
2. Dans la section **Modules**, cocher : Suivi des demandes, Wiki, Documents, Suivi du temps, Fichiers
3. Sauvegarder

Pour le sous-projet : répéter l'opération, puis dans le champ **Projet parent** sélectionner `TMA Capgemini Engineering`.

Vérifier dans la liste des projets que `tma-capgemini-batch` apparaît en retrait sous `tma-capgemini`.

:::{note}
L'identifiant est immutable après création — bien le vérifier avant de sauvegarder.
:::
::::

---

::::{exercise} EX-09 — Affectation des membres et des rôles
:label: redmine_ex09
:nonumber:

**Durée** : 15 min  
**Menu** : Projet → Paramètres → Membres

| Compte | Rôle |
|---|---|
| `cda.dupont` | Chef de projet |
| `dev.martin` | Développeur |
| `client.durand` | Client |

Vérifier en changeant de compte que chaque utilisateur accède uniquement aux fonctionnalités de son rôle.

**Critère de réussite** : chaque compte accède au projet avec les bonnes restrictions
::::

::::{solution} redmine_ex09
:label: sol_redmine_ex09
:class: dropdown

Menu : Projet `tma-capgemini` → **Paramètres** → onglet **Membres** → **Nouveau membre**

1. Sélectionner l'utilisateur dans la liste déroulante
2. Sélectionner le rôle correspondant
3. Cliquer **Ajouter** — répéter pour les 3 comptes

**Vérification par compte** :

| Compte | Ce qui doit être visible | Ce qui ne doit pas l'être |
|---|---|---|
| `cda.dupont` | Paramètres projet, tous les tickets, tous les temps | Administration |
| `dev.martin` | Ses tickets, saisie de temps | Paramètres projet, gestion membres |
| `client.durand` | Liste des tickets, création de demande | Temps des autres, paramètres |

:::{note}
Le sous-projet `tma-capgemini-batch` hérite des membres du projet parent par défaut — vérifier que c'est bien le cas.
:::
::::

---

::::{exercise} EX-10 — Création des versions (jalons)
:label: redmine_ex10
:nonumber:

**Durée** : 10 min  
**Menu** : Projet → Paramètres → Versions

| Version | Échéance |
|---|---|
| Sprint 1 | Fin de la semaine courante |
| Sprint 2 | + 2 semaines |

**Critère de réussite** : 2 versions affichées sur la roadmap du projet
::::

::::{solution} redmine_ex10
:label: sol_redmine_ex10
:class: dropdown

Menu : Projet → **Paramètres** → onglet **Versions** → **Nouvelle version**

Pour chaque version :
1. Saisir le nom (`Sprint 1`, `Sprint 2`)
2. Définir la date d'échéance
3. Statut : Ouvert
4. Sauvegarder

Vérifier via Menu : Projet → **Roadmap** — les 2 versions doivent apparaître avec leur date et le compteur de demandes (0 pour l'instant).

:::{note}
Une version en statut "Fermé" n'accepte plus de nouvelles demandes. Ne fermer qu'en fin de sprint.
:::
::::

---

::::{exercise} EX-11 — Création d'un lot de demandes fil rouge
:label: redmine_ex11
:nonumber:

**Durée** : 30 min  
**Menu** : Projet → Nouveau ticket (répété)

| Tracker | Sujet | Priorité | Assigné à | Version |
|---|---|---|---|---|
| Anomalie | Erreur 500 sur le module facturation | Urgente | dev.martin | Sprint 1 |
| Anomalie | Calcul TVA incorrect sur devis PDF | Haute | dev.martin | Sprint 1 |
| Évolution | Ajout export Excel sur liste commandes | Normale | — | Sprint 2 |
| Demande de service | Réinitialisation mot de passe utilisateur Dupont | Normale | dev.martin | Sprint 1 |
| Tâche interne | Mise à jour documentation API | Basse | dev.martin | Sprint 2 |
| Anomalie | Timeout connexion base de données | Urgente | dev.martin | Sprint 1 |

**Critère de réussite** : 6 demandes visibles, filtrables par tracker et par version
::::

::::{solution} redmine_ex11
:label: sol_redmine_ex11
:class: dropdown

Menu : Projet → **Nouveau ticket**

Pour chaque demande, renseigner dans l'ordre : Tracker, Sujet, Priorité, Assigné à, Version cible.

**Vérification** : Menu Projet → Suivi des demandes → appliquer les filtres suivants :

- Filtre `Version = Sprint 1` → doit retourner 4 demandes
- Filtre `Tracker = Anomalie` → doit retourner 3 demandes
- Filtre `Priorité = Urgente` → doit retourner 2 demandes

:::{note}
Si les champs personnalisés EX-11-ADV ont été créés en M1, renseigner aussi `Environnement` et `Criticité métier` sur les anomalies.
:::
::::

---

::::{exercise} EX-12 — Mise en place de la documentation projet (Wiki)
:label: redmine_ex12
:nonumber:

**Durée** : 20 min  
**Menu** : Projet → Wiki

Créer les pages suivantes :

| Page | Contenu attendu |
|---|---|
| Accueil | Présentation du projet, contacts, liens utiles |
| Procedure-Incident | Procédure de qualification d'une anomalie (3 étapes minimum) |
| Contacts | Tableau fictif des interlocuteurs (nom, rôle, email) |

Depuis la page Accueil, créer un lien vers la procédure :

```
[[Procedure-Incident]]
```

**Critère de réussite** : wiki navigable, lien fonctionnel entre les pages
::::

::::{solution} redmine_ex12
:label: sol_redmine_ex12
:class: dropdown

Menu : Projet → **Wiki** → **Modifier** (page Accueil générée automatiquement)

**Contenu de la page Accueil** (exemple) :

```
h1. TMA Capgemini Engineering

Bienvenue sur le wiki du projet TMA.

* [[Procedure-Incident|Procédure de gestion des incidents]]
* [[Contacts|Annuaire des interlocuteurs]]
```

**Contenu de la page Procedure-Incident** :

```
h1. Procédure de qualification d'une anomalie

h2. Étape 1 — Réception
Vérifier la complétude du signalement : environnement, reproductibilité, impact.

h2. Étape 2 — Qualification
Affecter un tracker, une priorité et une version cible.

h2. Étape 3 — Assignation
Affecter à un développeur et passer le statut en "En cours".
```

**Contenu de la page Contacts** :

```
| Nom | Rôle | Email |
| Dupont | Chef de projet | cda.dupont@example.com |
| Martin | Développeur | dev.martin@example.com |
| Durand | Client | client.durand@example.com |
```

Vérifier que le lien `[[Procedure-Incident]]` depuis la page Accueil est cliquable et navigue vers la bonne page.
::::

---

::::{exercise} EX-13 — Consultation et analyse des temps (simulation)
:label: redmine_ex13
:nonumber:

**Durée** : 20 min

**Saisie des temps** — chaque stagiaire saisit 3 entrées :

| Demande | Durée | Activité |
|---|---|---|
| Erreur 500 sur le module facturation | 2h | Développement |
| Réinitialisation mot de passe | 0,5h | Support / Assistance |
| Mise à jour documentation API | 1h | Documentation |

**Analyse** — Menu : Projet → Suivi du temps

- Filtrer par activité
- Filtrer par utilisateur
- Filtrer par version
- Identifier le total d'heures sur Sprint 1

**Critère de réussite** : rapport affiche les temps saisis, tous les filtres fonctionnels
::::

::::{solution} redmine_ex13
:label: sol_redmine_ex13
:class: dropdown

**Saisie** : Menu Projet → Suivi du temps → **Nouvelle entrée de temps**

Pour chaque entrée : sélectionner la demande, saisir la durée (format `2` ou `2.5`), choisir l'activité, ajouter un commentaire optionnel.

**Résultats attendus après filtrage** :

| Filtre | Résultat attendu |
|---|---|
| Activité = Développement | 2h sur "Erreur 500" |
| Activité = Support / Assistance | 0,5h sur "Réinitialisation MDP" |
| Version = Sprint 1 | 2,5h (Erreur 500 + Réinitialisation MDP) |
| Utilisateur = dev.martin | Total des 3 entrées = 3,5h |

:::{note}
Le rapport de temps est exportable en CSV via le bouton en bas de page — utile pour une restitution client.
:::
::::

---

## Exercices avancés

---

::::{exercise} EX-ADV-M2-01 — Requêtes et filtres personnalisés
:label: redmine_ex_adv_m2_01
:nonumber:

**Durée** : 20 min  
**Menu** : Projet → Suivi des demandes

Créer et sauvegarder 3 filtres réutilisables :

| Nom du filtre | Critères |
|---|---|
| Anomalies urgentes Sprint 1 | Tracker = Anomalie ET Priorité = Urgente ET Version = Sprint 1 ET Statut ≠ Fermé |
| Mes demandes en cours | Assigné à = moi ET Statut = En cours |
| Backlog Sprint 2 | Version = Sprint 2 ET Statut = Nouveau |

Pour chaque filtre :
1. Appliquer les critères via **Ajouter un filtre**
2. Choisir les colonnes à afficher (Tracker, Sujet, Priorité, Assigné à, Mis à jour)
3. Cliquer **Sauvegarder** — nommer le filtre
4. Cocher **Public** pour le partager à tous les membres du projet

**Critère de réussite** : 3 filtres sauvegardés, accessibles depuis le menu déroulant, le filtre public visible par `dev.martin` et `client.durand`
::::

::::{solution} redmine_ex_adv_m2_01
:label: sol_redmine_ex_adv_m2_01
:class: dropdown

Menu : Projet → Suivi des demandes → section **Filtres** → **Ajouter un filtre**

**Anomalies urgentes Sprint 1** :
- Tracker → est → Anomalie
- Priorité → est → Urgente
- Version → est → Sprint 1
- Statut → n'est pas → Fermé

Cliquer **Appliquer**, puis **Sauvegarder** → saisir le nom → cocher **Public** → **Sauvegarder**.

**Vérification** : se connecter avec `dev.martin` → Projet → Suivi des demandes → le filtre public doit apparaître dans la liste déroulante des requêtes sauvegardées.

:::{note}
Les filtres privés (non cochés Public) ne sont visibles que par leur créateur. Utiles pour les vues personnalisées de chaque développeur.
:::
::::

---

::::{exercise} EX-ADV-M2-02 — Roadmap et suivi d'avancement
:label: redmine_ex_adv_m2_02
:nonumber:

**Durée** : 20 min  
**Menu** : Projet → Roadmap

L'objectif est de simuler l'avancement d'un sprint et d'en lire les indicateurs.

**Étapes** :

1. Passer 2 demandes du Sprint 1 en statut "Résolu" (`dev.martin`)
2. Passer 1 demande en statut "Fermé" (`cda.dupont` ou `client.durand`)
3. Consulter la Roadmap : observer la barre de progression du Sprint 1
4. Consulter le Gantt : Menu Projet → Gantt
5. Répondre aux questions suivantes :
   - Quel pourcentage d'avancement affiche le Sprint 1 ?
   - Combien de demandes restent ouvertes ?
   - Le Sprint 2 apparaît-il dans le Gantt ?

**Critère de réussite** : roadmap affiche un avancement > 0%, Gantt affiche les 2 versions avec leurs demandes
::::

::::{solution} redmine_ex_adv_m2_02
:label: sol_redmine_ex_adv_m2_02
:class: dropdown

**Passage en Résolu** : se connecter avec `dev.martin` → ouvrir la demande → modifier le statut → Sauvegarder.

**Passage en Fermé** : se connecter avec `cda.dupont` → ouvrir la demande résolue → modifier le statut → Sauvegarder.

**Lecture de la Roadmap** :

- La barre de progression = (demandes fermées / total demandes de la version) × 100
- Avec 1 demande fermée sur 4 du Sprint 1 → **25%**
- Les demandes encore ouvertes sont listées sous la barre

**Lecture du Gantt** :

- Chaque demande apparaît comme une barre horizontale positionnée entre sa date de création et son échéance de version
- Le Sprint 2 apparaît si ses demandes ont une date d'échéance définie

:::{note}
Redmine calcule la progression de la roadmap uniquement sur les demandes à statut "fermé" — les demandes "Résolues" ne comptent pas encore. C'est un point important à expliquer aux chefs de projet.
:::
::::

---

::::{exercise} EX-ADV-M2-03 — API REST : gestion de projet
:label: redmine_ex_adv_m2_03
:nonumber:

**Durée** : 30 min

Utiliser l'API REST pour piloter le projet sans passer par l'interface web.

Remplacer `VOTRE_CLE_API` par la clé obtenue en EX-09-ADV (M1).

*1. Lister les membres du projet*

```powershell
curl -H "X-Redmine-API-Key: VOTRE_CLE_API" `
     http://localhost:3000/projects/tma-capgemini/memberships.json
```

*2. Créer une nouvelle version via API*

```powershell
curl -X POST `
     -H "X-Redmine-API-Key: VOTRE_CLE_API" `
     -H "Content-Type: application/json" `
     -d "{\"version\": {\"name\": \"Sprint 3\", \"status\": \"open\", \"due_date\": \"2026-07-31\"}}" `
     http://localhost:3000/projects/tma-capgemini/versions.json
```

*3. Lister les versions du projet*

```powershell
curl -H "X-Redmine-API-Key: VOTRE_CLE_API" `
     http://localhost:3000/projects/tma-capgemini/versions.json
```

*4. Affecter une demande existante à la nouvelle version*

Récupérer l'`id` du Sprint 3 dans la réponse de l'étape 3, puis :

```powershell
curl -X PUT `
     -H "X-Redmine-API-Key: VOTRE_CLE_API" `
     -H "Content-Type: application/json" `
     -d "{\"issue\": {\"fixed_version_id\": ID_SPRINT3}}" `
     http://localhost:3000/issues/ID_DEMANDE.json
```

*5. Fermer une version (fin de sprint)*

```powershell
curl -X PUT `
     -H "X-Redmine-API-Key: VOTRE_CLE_API" `
     -H "Content-Type: application/json" `
     -d "{\"version\": {\"status\": \"closed\"}}" `
     http://localhost:3000/versions/ID_SPRINT1.json
```

**Critère de réussite** : Sprint 3 créé et visible dans la roadmap, demande réaffectée, Sprint 1 fermé via API et non modifiable dans l'interface
::::

::::{solution} redmine_ex_adv_m2_03
:label: sol_redmine_ex_adv_m2_03
:class: dropdown

**Réponse attendue à l'étape 2 (création version)** :

```json
{
  "version": {
    "id": 3,
    "project": {"id": 1, "name": "TMA Capgemini Engineering"},
    "name": "Sprint 3",
    "status": "open",
    "due_date": "2026-07-31"
  }
}
```

L'`id` retourné (ici `3`) est à utiliser aux étapes 4 et 5.

**Étape 4** — remplacer `ID_SPRINT3` par `3` et `ID_DEMANDE` par l'id d'une demande existante (récupérable via `issues.json`) :

```powershell
curl -X PUT `
     -H "X-Redmine-API-Key: VOTRE_CLE_API" `
     -H "Content-Type: application/json" `
     -d "{\"issue\": {\"fixed_version_id\": 3}}" `
     http://localhost:3000/issues/5.json
```

**Étape 5** — récupérer l'id du Sprint 1 via l'étape 3 :

```powershell
curl -H "X-Redmine-API-Key: VOTRE_CLE_API" `
     http://localhost:3000/projects/tma-capgemini/versions.json
```

Puis fermer :

```powershell
curl -X PUT `
     -H "X-Redmine-API-Key: VOTRE_CLE_API" `
     -H "Content-Type: application/json" `
     -d "{\"version\": {\"status\": \"closed\"}}" `
     http://localhost:3000/versions/1.json
```

**Vérification** : dans l'interface, Menu Projet → Paramètres → Versions — le Sprint 1 apparaît en statut "Fermé" et n'accepte plus de nouvelles demandes.

**Erreurs fréquentes** :
- `422` sur la fermeture de version → des demandes ouvertes sont encore affectées à cette version
- `404` sur `/versions/ID` → l'id est incorrect, vérifier via `versions.json`
::::
